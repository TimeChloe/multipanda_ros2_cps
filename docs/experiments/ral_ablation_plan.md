...
4. Verify robot/tool enclosure coverage against actual MuJoCo geometry across configurations. Since v14, control and tool geometry share the ball-center TCP and the provider appends a TCP sphere using the configured radius; arm capsules remain enabled. Archive the workspace SaRA configuration as well as generated tool models. A zero secure radius needs demonstrated containment; energy margins do not cover geometric error. See [TCP migration](tcp_ball_center.md) before reusing earlier task goals.
