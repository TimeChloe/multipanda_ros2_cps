...
// === 与上一版相比的关键改动 ============================================
//
// 1. 误差管道 (tracking error tube)
//    旧版用一个二阶 ODE 在 horizon 上演化 r_p, r_v，但 ODE 系数让它不稳定
//      ṙ_p = r_v
//      ṙ_v = +α·r_p − β·r_v + γ      (α=60, λ_unstable = +2.65 rad/s)
//    在 7s 长的 Ruckig 轨迹上把误差包络放大到 r_p≈557m 这种荒谬值。
//
//    新版直接用两个**常数**作为 tracking error 包络:
//      r_p ≡ tracking_pos_error_bound_   (e.g. 0.005 m)
//      r_v ≡ tracking_vel_error_bound_   (e.g. 0.05  m/s)
//    含义: 真实机器人状态 x_true(t) 落在以预测 x_pred(t) 为中心、半径
//    r_p 的球内 (速度类似)。这是 reachset conformance 给出的稳态包络。
//
// 2. 验证范围 (intended horizon for verification)
//    旧版每周期 verifyCandidatePlan 把整段 Ruckig 输出 (~7000 步) 都跑了。
//    新版每周期只验证: 1 步 intended + N_fs 步 failsafe。
//    Ruckig 输出存到 intended_buffer_ 里, 验证只取 buffer[index] 的下一步。
//
// 3. Failsafe 锚点 (freeze position)
//    旧版 freeze_anchor = intended.back() (Ruckig 末端), 所以 failsafe 是
//    "如果机器人开到 7s 后再失败再去冻在那" — 与每周期单步执行不一致。
//    新版 freeze_anchor = 当前周期要执行那一步 intended 的终点位置 —
//    这才是 path-consistent 单步监控正确的 anchor。
//
// 4. invariably safe state 论证 (为什么不需要 horizon 跑到 v=0)
//    冻结 freeze 在 intended 段末位 x* 后, K_f 与 D_f 给出
//      E(t) = 0.5*M*||v||² + 0.5*K_f*||x - x*||²,    Ė ≤ 0
//    由于 x(t_M) = x* ⇒ V(t_M) = 0 ⇒ E(t_M) = T(t_M) = 0.5*M*||v(t_M)||²
//    任意 t > t_M:
//      T(t) ≤ E(t) ≤ E(t_M)              (动能上界)
//      ||x(t)-x*|| ≤ ||v(t_M)||*sqrt(M/K_f)  (位置最大游程)
//    所以 horizon 只需让 ||v(t_M)|| 足够小到这两个上界都满足安全裕度即可。
// =====================================================================
...
// ============================================================================
// Helper: matrix rate limit & runtime gain update (unchanged)
// ============================================================================
...
// path retiming helpers (unchanged) ----------------------------------------
...
// ============================================================================
// RViz diagnostics (unchanged - omitted body for brevity, keep your original)
// ============================================================================
...
// ============================================================================
// Interface configurations (unchanged)
// ============================================================================
...
// ============================================================================
// Sample factory helpers (unchanged signatures)
// ============================================================================
...
ImpedanceSample ReachableCartesianImpedanceController::makeFrozenFailsafeSample(
  ...
  ImpedanceSample s;
  s.t = nominal_time;
  s.p = freeze_sample.p;     // freeze position == intended end-of-step position
...
// ============================================================================
// Cache for Ruckig output  (NEW - replaces "verify whole intended buffer")
// last_verified_plan_.intended  is now always size==1 (single-step verified)
// intended_buffer_              holds the full Ruckig output (NOT yet verified)
// ============================================================================
ImpedanceSample ReachableCartesianImpedanceController::getNextIntendedCommandFromCache(
    bool advance_index) {
  // last_verified_plan_.intended[0] is always the single verified step
  ...
  // advance_index is kept for compatibility with the failsafe path; no-op here
  (void)advance_index;
  return cmd;
}
...
// (previously buildRuckigIntendedSegment populated plan.intended directly;
// now we drop them into intended_buffer_ for per-step verification)
// ============================================================================
...
// ============================================================================
// SARA-CHANGE: build a SINGLE-STEP candidate plan for verification
//   intended  = [next_step_from_buffer]                (D = 1)
//   failsafe  = N_fs samples freezing at intended[0].p (path-consistent w.r.t.
//                                                      end-of-intended pose)
// ============================================================================
VerifiedPlan ReachableCartesianImpedanceController::buildSingleStepCandidatePlan(
  ...
  // Anchor = where we ARE right now (real measured state)
  plan.anchor.t  = 0.0;
  ...
  // Failsafe: freeze at the END of THIS verified intended step
  // (path-consistent with the step we are about to execute)
...
// ============================================================================
// SARA-CHANGE: verifyCandidatePlan, NOW with constant tracking-error tube
// (no more unstable ODE for r_p, r_v).
// ============================================================================
MonitorResult ReachableCartesianImpedanceController::verifyCandidatePlan(
  ...
  // ---------------------------------------------------------------------
  // SARA-CHANGE: tracking-error tube is now CONSTANT (no ODE propagation)
  //   Rationale: closed-loop is stable. Tracking error is bounded by a
  //   steady-state envelope from controller calibration (or reachset
  //   conformance).  Keeping it constant means horizon length cannot
  //   blow it up, and the bound is independent of intended horizon.
  // ---------------------------------------------------------------------
  ...
  auto eval_sample = [&](const ImpedanceSample& s, double dtp) {
    ...
    if (use_dynamic_consistent_impedance_) {
      a_pred = s.ddp + Kp * pos_error_des_minus_cur - Dp * vel_error_cur_minus_des;
    } else {
      const Vector3d force_pred = Kp * pos_error_des_minus_cur - Dp * vel_error_cur_minus_des;
    ...
    const double k_n = computeNormalStiffnessAlongNormal(K_exec);
    const double d_n = computeNormalDampingAlongNormal(D_exec);

    double a_pos = 0.0, a_brake_raw = 0.0;
    if (use_dynamic_consistent_impedance_) {
      a_pos = k_n * e_n_abs +
              torque_to_accel_gain_ * tau_rate_limit_ * kMinDt +
              std::max(model_accel_uncertainty_, 0.0);
      a_brake_raw = d_n * v_n_abs - 0.5 * torque_to_accel_gain_ * tau_rate_limit_ * kMinDt;
    } else {
      a_pos = computeConservativeNormalAccelPositiveBound(m_eff_step_n, e_n_abs, v_n_abs, K_exec);
      a_brake_raw = computeConservativeNormalBrakeAccelLowerBound(m_eff_step_n, v_n_abs, D_exec);
    }
...
// ============================================================================
// SARA-CHANGE: shield decision is now per-cycle, single-step verification
// ============================================================================
ShieldDecision ReachableCartesianImpedanceController::computeShieldDecision(
  ...
  if (dec.candidate_verified) {
    // SUCCESS: cache as last_verified_plan_, advance buffer, execute step
  ...
  // First verification failed. Try once more after FORCING a Ruckig refresh
  // (perhaps the cached lookahead became stale w.r.t. current human state).
  intended_buffer_valid_ = false;
  intended_buffer_index_ = 0;
  ...
  // Both attempts failed -> execute last verified failsafe (or emergency)
  mode_ = SafetyMode::kFailsafe;
...
Vector7d ReachableCartesianImpedanceController::computeImpedanceTorque(
  ...
  xdot_des.head<3>() = cmd.dp;  xdot_des.tail<3>() = cmd.w;
  Vector6d xddot_des = Vector6d::Zero();
  xddot_des.head<3>() = cmd.ddp; xddot_des.tail<3>() = cmd.dw;
  ...
  if (use_dynamic_consistent_impedance_) {
    ...
    const Vector6d wrench = lambda * (xddot_des - K_runtime_ * error - D_runtime_ * xdot_error);
    tau_task = J_geo.transpose() * wrench;
  } else {
    tau_task = J_geo.transpose() * (-K_runtime_ * error - D_runtime_ * xdot_error);
  }
  ...
  for (int i = 0; i < 7; ++i) {
    const double delta = std::clamp(tau_des(i) - tau_cmd_prev_(i), -max_delta, max_delta);
...
// ============================================================================
// update() — same structure; nominal_guess_time / paused-time bookkeeping kept
// ============================================================================
controller_interface::return_type ReachableCartesianImpedanceController::update(
  ...
  if (do_monitor) {
    ...
    if (mode_ == SafetyMode::kFailsafe || shield_dec.executing_last_verified_failsafe) {
      ...
      // Between monitor calls we just reissue the latest verified intended cmd
      if (last_verified_plan_.valid && !last_verified_plan_.intended.empty())
...
// ============================================================================
// on_init: declare new params, remove the unstable-ODE params
// ============================================================================
CallbackReturn ReachableCartesianImpedanceController::on_init() {
  try {
    ...
    // SARA-CHANGE: new constant tracking-error bounds (replace alpha/beta/gamma)
    auto_declare<double>("tracking_pos_error_bound", 0.005);   // 5 mm
    auto_declare<double>("tracking_vel_error_bound", 0.05);    // 5 cm/s

    // SARA-CHANGE: longer failsafe horizon so freeze impedance has time to settle
    //   shield_horizon_steps * shield_plan_dt should cover ~3-5 settling
    //   time-constants of the K_f / D_f closed loop.
    auto_declare<int>("shield_horizon_steps", 100);            // 100*0.01=1.0s
    ...
    auto_declare<int>("local_replan_horizon_steps", 200);      // hard cap on Ruckig steps
...
// ============================================================================
// on_configure: read new params, drop unstable-ODE params
// ============================================================================
CallbackReturn ReachableCartesianImpedanceController::on_configure(
  ...
  try {
    ...
    // SARA-CHANGE: constant tracking-error bounds
